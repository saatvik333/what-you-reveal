...
export async function collectHardwareData() {
    ...
    if (typeof navigator.getBattery === 'function') {
        try {
            ...
            hardwareData['Charging Time'] = battery.chargingTime === Infinity ? 'N/A' : (battery.chargingTime / 60).toFixed(0) + ' min';
            hardwareData['Discharging Time'] = battery.dischargingTime === Infinity ? 'N/A' : (battery.dischargingTime / 60).toFixed(0) + ' min';
        } catch (e) {
    ...
    try {
        if (performance && performance.memory) {
            hardwareData['JS Heap Size Limit'] = (performance.memory.jsHeapSizeLimit / 1048576).toFixed(0) + ' MB';
            hardwareData['Total JS Heap Size'] = (performance.memory.totalJSHeapSize / 1048576).toFixed(0) + ' MB';
            hardwareData['Used JS Heap Size'] = (performance.memory.usedJSHeapSize / 1048576).toFixed(0) + ' MB';
        }
    } catch(e) { /* ignore */ }
    ...
    hardwareData['Device Orientation API'] = 'DeviceOrientationEvent' in window ? 'Supported' : 'Not Supported';
